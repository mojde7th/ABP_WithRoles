...
namespace TodoApp.Application
{
    public class AccountAppService:ApplicationService, IAccountAppService
    {
      ...
      public async Task<DailyPlanDto> CreateDailyPlanAsync
            ...
            (Guid id)
        {
                ...
                foreach(var section in dailyPlanDto.Sections)
            {
                ...
                section.TotalLayerDuration = section.PlanLayers.Sum
                    (l => l.Duration);
                section.TotalDuration = section.TotalTaskDuration 
                    + section.TotalLayerDuration;
        ...
        public async Task<List<DailyPlanDto>> GetAllDailyPlansAsync()
        {
            ...
            foreach(var dailyPlan in dailyPlanDtos)
            {
                ...
                foreach (var section in dailyPlan.Sections)
                {
                    ...
                    section.CumulativeTaskDuration = cumulativeTaskDuration;
                    section.CumulativeLayerDuration = cumulativeLayerDuration;
                    section.CumulativeTotalDuration = cumulativeLayerDuration +
        ...
        public async Task<DailyPlanDto> CreateFullDailyPlanAsync(CreateFullDailyplanDto input)
        {
            ...
            foreach (var sectionDto in input.Sections)
            {
                DetachTrackedEntities();
                var section = new Section
                {
                ...
                {
                    DetachTrackedEntities();
                    // Ensure each task has a new unique Id generated by EF Core
                    var task = new Task
                    {
                        Name = taskDto.Name,
                        Duration = taskDto.Duration,
                        Section = section  // Link task to the section
                    };
                    section.Tasks.Add(task); // Add task to section
                }

                foreach (var layerDto in sectionDto.PlanLayers)
                {
                    DetachTrackedEntities();
                    // Ensure each plan layer has a new unique Id generated by EF Core
                    var planLayer = new PlanLayer
                    {
                        Name = layerDto.Name,
                        Duration = layerDto.Duration,
                        Section = section  // Link layer to the section
                    };
                    section.PlanLayers.Add(planLayer); // Add layer to section
                }

                dailyPlan.Sections.Add(section); // Add section to daily plan
            }
        ...
        // Detach all tracked entities before processing new ones to avoid conflicts
        public void DetachTrackedEntities()
        {
            var entries = _todoAppDbContext.ChangeTracker.Entries().ToList();
